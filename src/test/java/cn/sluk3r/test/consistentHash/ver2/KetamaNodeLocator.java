package cn.sluk3r.test.consistentHash.ver2;

import cn.sluk3r.test.consistentHash.self.Node;

import java.util.List;
import java.util.SortedMap;
import java.util.TreeMap;


public final class KetamaNodeLocator {
	
	private TreeMap<Long, Node> consistentHashLoop;
	private HashAlgorithm hashAlg;
	private int numReps = 160;
	
    public KetamaNodeLocator(List<Node> nodes, HashAlgorithm alg, int nodeCopies) {
		hashAlg = alg;
		consistentHashLoop =new TreeMap<Long, Node>();
		
        numReps= nodeCopies;
        
		for (Node node : nodes) {
			for (int i = 0; i < numReps / 4; i++) {
				byte[] digest = hashAlg.computeMd5(node.getName() + i);
				for(int h = 0; h < 4; h++) {
					long m = hashAlg.hash(digest, h);

					//这个Key是基于Node的名字计算出来的。
					consistentHashLoop.put(m, node);
				}
			}
		}
    }

	public Node getPrimary(final String k) {
		byte[] digest = hashAlg.computeMd5(k);
		Node rv=getNodeForKey(hashAlg.hash(digest, 0));
		return rv;
	}

	Node getNodeForKey(long hash) {
		final Node rv;
		Long key = hash;
		if(!consistentHashLoop.containsKey(key)) {
			SortedMap<Long, Node> tailMap= consistentHashLoop.tailMap(key);
			if(tailMap.isEmpty()) {
				key= consistentHashLoop.firstKey(); //为什么是firstKey？
			} else {
				key=tailMap.firstKey();
			}
			//For JDK1.6 version
//			key = consistentHashLoop.ceilingKey(key);
//			if (key == null) {
//				key = consistentHashLoop.firstKey();
//			}
		}

		rv= consistentHashLoop.get(key);
		return rv;
	}
}
