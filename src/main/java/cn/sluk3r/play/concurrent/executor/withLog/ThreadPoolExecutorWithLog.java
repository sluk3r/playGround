...
public class ThreadPoolExecutorWithLog extends AbstractExecutorService {
    private static final Logger logger = Logger.getLogger(ThreadPoolExecutorWithLog.class);
     ...
     */
    private volatile RejectedExecutionHandlerWithLog handler;
     ...
     */
    private static final RejectedExecutionHandlerWithLog defaultHandler =
            new AbortPolicyWithLog();
     ...
     */
    public ThreadPoolExecutorWithLog(int corePoolSize,
                              int maximumPoolSize,
                              long keepAliveTime,
                              TimeUnit unit,
    ...
    public ThreadPoolExecutorWithLog(int corePoolSize,
                              ...
                              TimeUnit unit,
                              BlockingQueue<Runnable> workQueue,
                              RejectedExecutionHandlerWithLog handler) {
        this(corePoolSize, maximumPoolSize, keepAliveTime, unit, workQueue,
    ...
    public void execute(Runnable command) {
        if (command == null)
            throw new NullPointerException();
//        logger.info(String.format("before execute>>>>>>>>>>>>>>>poolSize:%s, corePoolSize:%s, maximumPoolSize:%s, queue size:%s", poolSize, corePoolSize, maximumPoolSize, workQueue.size()));
        int poolSizeBefore = poolSize;
        int corePoolSizeBefore = corePoolSize;
        int maximumPoolSizeBefore = maximumPoolSize;
        int queueSizeBefore = workQueue.size();
        if (poolSize >= corePoolSize || !addIfUnderCorePoolSize(command)) {
//            if (runState == RUNNING ) {
//                logger.info("Executor RUNNING");
//                boolean workQueueOfferResult  = workQueue.offer(command);
//                logger.info("workQueueOfferResult: " + workQueueOfferResult);
//                if (workQueueOfferResult) {
...
//                reject(command); // is shutdown or saturated
        ...
        logger.info(String.format("execute===================poolSize:[%s][%s], corePoolSize:[%s][%s], maximumPoolSize:[%s][%s], queue size:[%s][%s]",
                poolSizeBefore,poolSize, corePoolSizeBefore, corePoolSize, maximumPoolSizeBefore,  maximumPoolSize, queueSizeBefore, workQueue.size()));
    }
    ...
    private Thread addThread(Runnable firstTask) {
        ...
        if (t != null) {
            ...
            w.thread = t;
            workers.add(w);
            logger.info("about to increment poolSize");
            int nt = ++poolSize;
    ...
    private boolean addIfUnderCorePoolSize(Runnable firstTask) {
        ...
        mainLock.lock();
        try {
//            logger.info(String.format("poolSize:%s, corePoolSize:%s, maximumPoolSize:%s", poolSize, corePoolSize, maximumPoolSize));
    ...
    private void ensureQueuedTaskHandled(Runnable command) {
        logger.info("ensureQueuedTaskHandled invoked");
        final ReentrantLock mainLock = this.mainLock;
        mainLock.lock();
     ...
     */
    public void setRejectedExecutionHandler(RejectedExecutionHandlerWithLog handler) {
        if (handler == null)
            throw new NullPointerException();
        this.handler = handler;
    }
     ...
     */
    public RejectedExecutionHandlerWithLog getRejectedExecutionHandler() {
        return handler;
    }
     ...
     * is discarded.
     */
    public static class CallerRunsPolicy implements RejectedExecutionHandlerWithLog {
        /**
         ...
         */
        public void rejectedExecution(Runnable r, ThreadPoolExecutorWithLog e) {
            if (!e.isShutdown()) {
                r.run();
            }
        }
     ...
     */
    public static class AbortPolicyWithLog implements RejectedExecutionHandlerWithLog {
        /**
         * Creates an <tt>AbortPolicy</tt>.
         */
        public AbortPolicyWithLog() { }
     ...
     * rejected task.
     */
    public static class DiscardPolicy implements RejectedExecutionHandlerWithLog {
        /**
     ...
     */
    public static class DiscardOldestPolicy implements RejectedExecutionHandlerWithLog {
        /**
